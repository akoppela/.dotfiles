...
#+begin_src emacs-lisp
      ...
      (when (and text
        ...
        (let* ((buffer (plist-get my/whisper-session :buffer))
          ...
          (with-current-buffer buffer
            (pcase (plist-get my/whisper-session :mode)
              ('terminal (vterm-send-string final-text))
              ('exwm (kill-new final-text)
                     (exwm-input--fake-key ?\C-v))
              ('buffer (when-let ((marker (plist-get my/whisper-session :marker)))
                         (goto-char marker))
                       (insert final-text)))
            (message "Text %s%s"
  ...
  (defun my/whisper-recording-p ()
    "Return t if whisper is currently recording."
    (and (boundp 'whisper--recording-process)
  ...
  (defmacro my/define-whisper-hydra (name language display-name)
    `(defhydra ,name (:color pink :hint nil :body-pre (unless (my/whisper-recording-p) (my/whisper ,language)))
       ,(format "
    Whisper %s [Refine: %%(if (and my/whisper-session (plist-get my/whisper-session :refine)) \"ON\" \"OFF\")]

    _t_: Toggle refine  _c_: Clean history  _q_: %%(if (my/whisper-recording-p) \"Stop & Transcribe\" \"Cancel\")
    " display-name)
       ("t" (when my/whisper-session
              (plist-put my/whisper-session :refine
                         (not (plist-get my/whisper-session :refine)))))
       ("c" (my/whisper-cleanup-history-buffers))
       ("q" (if (my/whisper-recording-p)
                (interrupt-process whisper--recording-process)
              (message "Cancelled"))
        :exit t)))

  (my/define-whisper-hydra my/whisper-english-hydra "en" "English")
  (my/define-whisper-hydra my/whisper-russian-hydra "ru" "Russian")
  ...
  (use-package whisper
    ...
    (general-def
      :states '(normal visual insert motion emacs)
      "s-w" 'my/whisper-english-hydra/body
      "s-W" 'my/whisper-russian-hydra/body)
    :config
