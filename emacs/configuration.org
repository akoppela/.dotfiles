...
#+begin_src emacs-lisp
  ...
  (defun my/conta-terminal (name location)
    ...
    (if (get-buffer name)
      ...
      (projectile-with-default-dir "~/contadev"
        (vterm)
        (my/vterm-send-string-and-return (concat "cd " location))
        (vterm-clear)
        (rename-buffer name))))
