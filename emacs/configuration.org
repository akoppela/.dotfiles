...
** Audio
   ...
   #+begin_src emacs-lisp
     (defconst my/volume-modifier "4")

     (defun my/audio-mute ()
       (interactive)
       (start-process "audio-mute" nil "pulsemixer" "--toggle-mute"))

     (defun my/audio-raise-volume ()
       (interactive)
       (start-process "raise-volume" nil "pulsemixer" "--change-volume" (concat "+" my/volume-modifier)))

     (defun my/audio-lower-volume ()
       (interactive)
       (start-process "lower-volume" nil "pulsemixer" "--change-volume" (concat "-" my/volume-modifier)))

     (general-def
       "<XF86AudioMute>" 'my/audio-mute
       "<XF86AudioRaiseVolume>" 'my/audio-raise-volume
       "<XF86AudioLowerVolume>" 'my/audio-lower-volume)
   #+end_src
   ...
   #+begin_src emacs-lisp
     (use-package exwm
       ...
       (when (eq window-system 'x)
         (require 'exwm-config)
         (exwm-config-default)
         (dolist (keys '(XF86AudioLowerVolume
                         XF86AudioRaiseVolume
                         XF86AudioMute))
           (cl-pushnew keys exwm-input-prefix-keys))))
   #+end_src
